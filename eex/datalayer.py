...
# Generic helper close/save/list/etc functions
    ...
    def set_box_size(self, lattice_const, utype=None):
        ...
        # Make sure we have all keywords that define a simulation box
        for k in dimensions:
            if k.lower() not in lattice_const and k.upper(
            ) not in lattice_const:
                raise KeyError("Could not find key '%s'." % k)
...
# Atom functions
    ...
    def _store_atom_table(self, table_name, df, property_name, by_value,
        ...
        if np.issubdtype(field_data["dtype"], np.integer):
...
# Term functions
    ...
    def get_term_count(self, order=None, uid=None):
        ...
        if uid is None:
            if order is None:
                raise KeyError(
                    "DataLayer:get_term_count: Cannot use 'uid' if 'order' is None."
                )
