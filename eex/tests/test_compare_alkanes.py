...
def build_dl(program, molecule):
    if program.lower() == "amber":
        ...
        fname = eex_find_files.get_example_filename("amber", "alkanes",
                                                    file_name)
        ...
        fname = eex_find_files.get_example_filename("lammps", "alkanes",
                                                    file_name)
...
def test_translation(program1, program2, molecule):
    ...
    original_atoms = original_dl.get_atoms(
        properties=["mass", "atom_type", "charge", "xyz"], by_value=True)
    ...
    assert (eex.testing.dict_compare(original_atoms.to_dict(),
                                     new_atoms.to_dict()))
    ...
    assert (eex.testing.dict_compare(original_dl_nb_parameters,
                                     new_dl2_nb_parameters))
