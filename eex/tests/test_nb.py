...
def test_convert_same_LJ_coeffs(form):
    ...
    assert np.allclose(new_coeffs[var_names[form][0]],
                       coeffs[var_names[form][0]])
    assert np.allclose(new_coeffs[var_names[form][1]],
                       coeffs[var_names[form][1]])
...
@pytest.mark.parametrize("mixing_rule",
                         ["lorentz_berthelot", "geometric", "sixthpower"])
def test_LJ_mixing(mixing_rule):
    ...
    new_coeffs = eex.nb_converter.mix_LJ(
        coeff1,
        coeff2,
        origin="epsilon/sigma",
