...
def test_compute_lattice_constants():
    ...
    lattice_const = eex.translators.lammps.lammps_utility.compute_lattice_constants(
        bsize, tilt_factors)
...
def test_fuzzy_list_match():
    ...
    assert (True, "data") == eex.utility.fuzzy_list_match(
        "data tmp", ["other", "data", "else"])
...
def test_hash():
    ...
    assert 'a1b8364f4dd47beb4cc0dd364d78d05d' == eex.utility.hash(
        ["something", 5])

    # Nested hash
    assert eex.utility.hash([5, (5, 6)]) == eex.utility.hash([5, [5, 6]])
    assert eex.utility.hash([5, (5, 6)]) == eex.utility.hash(
        [5, np.array([5, 6])])
    assert eex.utility.hash([[5], [6]]) == eex.utility.hash(
        np.array([[5], [6]]))
    ...
    assert eex.utility.hash([5.0, 6.0]) == eex.utility.hash(
        [5.0 + 1.e-10, 6.0 + 1.e-10])
    ...
    # Change tolerances
    assert eex.utility.hash([5.0], rtol=9) == eex.utility.hash([5.0 + 1.e-10],
                                                               rtol=9)
    assert eex.utility.hash([5.0], rtol=9) != eex.utility.hash([5.0 + 1.e-9],
                                                               rtol=9)

    # Fun gotchas
    assert eex.utility.hash([5.0], rtol=8) != eex.utility.hash([5.0], rtol=9)

    assert eex.utility.hash(("k", {
        "a": 5,
        "b": 6
    })) == eex.utility.hash(("k", {
        "b": 6,
