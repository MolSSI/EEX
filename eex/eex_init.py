...
for k, v in md.atom_metadata.items():
    if md.atom_metadata[k]["units"] is None:
        continue
    md.atom_metadata[k]["utype"] = {
        k: convert_contexts(v)
        for k, v in md.atom_metadata[k]["units"].items()
    }
    # md.atom_metadata[k]["utype"] = list(md.atom_metadata[k]["utype"].values())[0]

# Build the utype for the term data
for md_body in [
        md.two_body_metadata, md.three_body_metadata, md.four_body_metadata
]:
    for k, v in md_body["forms"].items():
        v["utype"] = {
            uk: convert_contexts(uv)
...
for k, v in md.nb_metadata["forms"].items():
    for uk, uv in v.items():
        ...
        uv["utype"] = {
            uuk: convert_contexts(uuv)
            for uuk, uuv in uv["units"].items()
        }
