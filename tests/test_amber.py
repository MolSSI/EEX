...
def test_amber_read():
    ...
    #
    # Check Atoms by_value
    atoms = dl.get_atoms(["atom_name", "charge", "atomic_number", "mass"], by_value=True)
    ...
    # Check Atoms by_index
    atoms = dl.get_atoms(["atom_name", "charge", "atomic_number", "mass", "residue_name", "residue_index"], by_value=False)
    assert atoms.shape[0] == 648
    assert set(np.unique(atoms["atom_name"])) == set([0, 1, 2])
